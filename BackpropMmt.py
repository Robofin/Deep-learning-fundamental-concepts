...
# BackpropMmt implements the delta rule
def BackpropMmt(W1, W2, X, D):
	...
	mmt1 = np.zeros((input_points))
	mmt2 = np.zeros((input_points))
	...
	for k in range(data_points):
		...
		for i in range(start, stop, input_points):
			...
			mmt1 = dW1 + beta*mmt1
			W1[0, i] = W1[0, i] + mmt1[0][0]
			W1[0, i+1] = W1[0, i+1] + mmt1[0][1]
			W1[0, i+2] = W1[0, i+2] + mmt1[0][2]


			dW2 = alpha * delta * np.transpose(y1)
			mmt2 = dW2 + beta*mmt2
			W2[0, i] = W2[0, i] + mmt2[0][0]
			W2[0, i+1] = W2[0, i+1] + mmt2[0][1]
			W2[0, i+2] = W2[0, i+2] + mmt2[0][2]
